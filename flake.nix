{
  ...
  outputs = { self, nixpkgs, nixpkgs-stable, nix-darwin, home-manager, hyprland, nixos-hardware, flake-utils, flake-parts, rust-overlay, nix-vscode-extensions, firefox-addons, sops-nix, dotfiles }:
    ...
    in
    {
      ...
      # Package sets are available via `import ./packages { inherit pkgs lib; }`
      # but not exported as flake packages (they are lists, not derivations)
      ...
      # Checks: nix flake check evaluates all configurations above.
      # Additional CI checks (gitleaks, builds) are in .github/workflows/ci.yml.
    };
}